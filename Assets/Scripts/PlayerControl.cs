...
public class PlayerControl : MonoBehaviour
{
    #region Variables
        ...
        private Transform currentPlatform; // Referencia a la plataforma en movimiento actual
    ...
    void LateUpdate()
    {
        // Si estamos en una plataforma en movimiento, ajusta la posición del jugador
                ...
                #region Air State
                    ...
                    case stAir:
                        ...
                        #region Hit Ground
                            ...
                            {
                                if (!hit.collider.CompareTag("Enemy")&&!hit.collider.CompareTag("Jumper")&&!hit.collider.CompareTag("Ring")) // Asegúrate de que el objeto no sea un "Enemy"
                                {
                                    HitGround();
                                    break;
                                }
    ...
    #region Collision
            void BounceOffEnemy()
        {
            ySpd = bounceSpeed*.8f; // Aplica la velocidad de rebote
            SetAirAccel(); // Configura el estado para el aire
            jumping = true; // Asegúrate de que el estado sea de salto

        }
            void BounceOffJumper()
        {
            ySpd = bounceSpeed*1.5f; // Aplica la velocidad de rebote
            SetAirAccel(); // Configura el estado para el aire
            jumping = true; // Asegúrate de que el estado sea de salto
...
void OnControllerColliderHit(ControllerColliderHit hit)
{
    ...
    {
        // Si el jugador no está ya en la plataforma, sincroniza su posición
    ...
    {
        // Si deja de estar en contacto con la plataforma, resetea las referencias
